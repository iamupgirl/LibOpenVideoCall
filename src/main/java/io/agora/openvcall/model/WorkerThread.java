...
public class WorkerThread extends Thread {
    ...
    public final void joinChannel(final String channel, int uid) {
        ...
        ensureRtcEngineReadyLock();
        String token = (String) SPUtils.get(mContext, Constant.TOKEN, "");
        if (TextUtils.isEmpty(token)) {
            Log.i("sherry", "joinChannel token=null");
            mRtcEngine.joinChannel(null, channel, "OpenVCall", uid);
            mRtcEngine.leaveChannel();
        } else {
            Log.i("sherry", "joinChannel token=" + token);
    ...
    private RtcEngine ensureRtcEngineReadyLock() {
        if (mRtcEngine == null) {
            String appId = (String) SPUtils.get(mContext.getApplicationContext(), Constant.APP_ID, "");
